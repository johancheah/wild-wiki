-- WILD Gaming Valorant Tracker — Phase 1 raw storage schema.
-- Populated directly from HenrikDev API responses. No derived/computed
-- columns live here (see PLAN.md §1) — that's Phase 2's job, computed
-- from kill_events/rounds into a separate derived_player_match_stats table.
...
-- headshot_filename is a file under static/headshots/, set manually per
...
CREATE TABLE IF NOT EXISTS matches (
    ...
    margin          INTEGER,
    raw_payload     TEXT,               -- full match-details-v4 JSON for API rows; NULL for spreadsheet rows
...
-- adr/hs_pct/role/fk/fd are nullable and populated ONLY for spreadsheet-
-- sourced rows, where we get the pre-computed value directly but not its
-- raw components (no per-shot or per-round data survives in the sheet).
-- API-sourced rows leave these null and compute ACS/ADR/HS% from the raw
-- counters (headshots/bodyshots/legshots/damage_dealt/rounds_played) in the
-- views layer instead; FK/FD for API rows is a Phase 2 kill_events derivation.
CREATE TABLE IF NOT EXISTS match_players (
    ...
    fk                    INTEGER,
    fd                    INTEGER,
    acs                   REAL,  -- spreadsheet rows only (direct); API rows compute from score/rounds_played in the view
    kast_pct              REAL,  -- spreadsheet rows only — genuinely unavailable from match-details-v4, not approximated for API rows
...
CREATE TABLE IF NOT EXISTS kill_events (
    ...
    assistant_ids        TEXT,   -- JSON array of player_ids
    weapon                TEXT,
    location_x           REAL,
    location_y           REAL,
